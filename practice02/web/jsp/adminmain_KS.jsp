...
<%
	...
	count = cc.getCount_KS_admin(searchword, keyword);
	
	ArrayList<ContactVO> contactlist = null;
	if(count>0){
		contactlist = cc.searchByKS_admin(keyword, searchword, startRow, endRow);
...
<div class="main_window">
	...
	<table>
		...
		%>
		<tr>
			<td colspan="7" align="center">연락처가 없습니다.</td>
		</tr>
		<%
			}
		%>
		<tr>
					<td colspan="7" align="center">
						...
						%>
									<a href="<%=request.getContextPath()%>/jsp/adminmain_KS.jsp?pageNum=<%=startPage - 10%>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[이전]</a>	
						<%			
								}
						...
						%>
										<a href="<%=request.getContextPath()%>/jsp/adminmain_KS.jsp?pageNum=<%=i%>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[<%=i %>]</a>
						<%	
									}
								} // for end
						...
						%>
									<a href="<%=request.getContextPath()%>/jsp/adminmain_KS.jsp?pageNum=<%=startPage + 10 %>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[다음]</a>
						<%			
								}
							}
						%>
