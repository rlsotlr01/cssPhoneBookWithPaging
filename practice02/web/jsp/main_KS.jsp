...
<%
	...
	count = cc.getCount_KS(id, searchword, keyword);
	
	ArrayList<ContactVO> contactlist = null;
	if(count>0){
		contactlist = cc.searchByKS(keyword, searchword, id, startRow, endRow);
	}
%>
	...
	<div id="main-box" class ="bgimg">
		...
		<tr>
					<td colspan="7" align="center">
						...
						%>
									<a href="<%=request.getContextPath()%>/jsp/main_KS.jsp?pageNum=<%=startPage - 10%>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[이전]</a>	
						<%			
								}
						...
						%>
										<a href="<%=request.getContextPath()%>/jsp/main_KS.jsp?pageNum=<%=i%>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[<%=i %>]</a>
						<%	
									}
								} // for end
						...
						%>
									<a href="<%=request.getContextPath()%>/jsp/main_KS.jsp?pageNum=<%=startPage + 10 %>&keyword=<%=keyword %>&searchword=<%=searchword %>" class="page">[다음]</a>
						<%			
								}
							}
						%>
